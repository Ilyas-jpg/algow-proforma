...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Length > 0 && e.Args[0] == "--test-quote")
        {
            try
            {
                ...
                var brand = AyTeknikKatalog.Models.Catalog.CreateDefault().Brand;
                var quote = new AyTeknikKatalog.Models.Quote
                {
                    ...
                    CustomerSalutation = AyTeknikKatalog.Models.Salutation.Bey,
                    ...
                    DiscountMode = AyTeknikKatalog.Models.DiscountMode.Yuzde,
                    DiscountValue = 5m,
                };
                quote.Lines.Add(new AyTeknikKatalog.Models.QuoteLine { Code = "CER0018", Name = "Fark Basınç Kontrol Vanası DN25", Description = "Pirinç gövde · 16 bar", Unit = "adet", Quantity = 10, UnitPrice = 1850m, VatRate = 20m });
                quote.Lines.Add(new AyTeknikKatalog.Models.QuoteLine { Code = "CER0011", Name = "Termovil Grubu 100 mm", Unit = "adet", Quantity = 25, UnitPrice = 145m, VatRate = 20m, DiscountPct = 10m });
                quote.Lines.Add(new AyTeknikKatalog.Models.QuoteLine { Code = "CER0001", Name = "Kör Tapa 1/2\"", Unit = "adet", Quantity = 200, UnitPrice = 18.5m, VatRate = 20m });
                quote.Lines.Add(new AyTeknikKatalog.Models.QuoteLine { Code = "CER0054", Name = "Rakor Grubu 3/4\"", Description = "Çift taraflı · conta dahil", Unit = "takım", Quantity = 40, UnitPrice = 92m, VatRate = 20m });
                var outPath = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "algow-teklif-ornek.pdf");
                AyTeknikKatalog.Services.QuotePdfService.Generate(quote, brand, outPath);
            }
            catch (Exception ex)
            {
    ...
    private static void RunCoverShowcase()
    {
        ...
        void Produce(string subFolder, string fileName, Action<AyTeknikKatalog.Models.Catalog> setup)
        {
            try
            {
                var cat = AyTeknikKatalog.Models.Catalog.CreateDefault();
                // 3 örnek ürün ekle (kapak sonrası içerik sayfası için)
                cat.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Topkapı Tam Geçişli Bilyalı Vana", Code = "ORN-V-001", Price = 449m, Currency = "TL" });
                cat.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Galata DN50 Küresel Vana", Code = "ORN-V-002", Price = 689m, Currency = "TL" });
                cat.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Boğaziçi Çek Valf 1/2\"", Code = "ORN-V-003", Price = 189m, Currency = "TL" });
                setup(cat);
                var dir = Path.Combine(root, subFolder);
                Directory.CreateDirectory(dir);
                new AyTeknikKatalog.Services.PdfService().Generate(cat, Path.Combine(dir, $"{fileName}.pdf"));
                produced++;
        ...
        foreach (var design in AyTeknikKatalog.Models.PdfDesign.All)
        ...
        if (hasCover)
        {
            foreach (var designId in new[] { "klasik", "sertifika", "minimalist" })
            {
                Produce("02b-foto-modlar", $"{designId}-FullPage", c =>
                {
                    ...
                    c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.FullPage;
                });
                Produce("02b-foto-modlar", $"{designId}-UstYari", c =>
                {
                    c.DesignId = designId;
                    c.Cover.CustomCoverImagePath = coverImagePath;
                    c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.TopWithBrandBar;
                });
                Produce("02b-foto-modlar", $"{designId}-SagYari", c =>
                {
                    c.DesignId = designId;
                    c.Cover.CustomCoverImagePath = coverImagePath;
                    c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.BackgroundWithOverlay;
                });
            }
        }
        else
        ...
        Produce("06-freeform-ornekler", "02-foto-arkaplan-bronz-overlay", c =>
        {
            ...
            c.Cover.Elements.Add(new AyTeknikKatalog.Models.CoverElement
            {
                Type = AyTeknikKatalog.Models.CoverElementType.Rectangle,
        ...
        foreach (var theme in AyTeknikKatalog.Models.PdfTheme.All)
    ...
    private static void RunStressTest()
    {
        ...
        void Run(string folder, string name, Action<AyTeknikKatalog.Models.Catalog> setup, Func<AyTeknikKatalog.Models.Catalog>? makeCat = null)
        {
            var sw = System.Diagnostics.Stopwatch.StartNew();
            try
            {
                var cat = makeCat?.Invoke() ?? AyTeknikKatalog.Models.Catalog.CreateDefault();
                setup(cat);
                ...
                new AyTeknikKatalog.Services.PdfService().Generate(cat, path);
                sw.Stop();
        ...
        void RunSingle(string folder, string name, Func<(AyTeknikKatalog.Models.Catalog cat, AyTeknikKatalog.Models.Product prod)> setup)
        {
            var sw = System.Diagnostics.Stopwatch.StartNew();
            try
            {
                var (cat, prod) = setup();
                var path = Path.Combine(root, folder, $"{name}.pdf");
                new AyTeknikKatalog.Services.PdfService().GenerateSingleProduct(cat, prod, path);
                sw.Stop();
                var size = File.Exists(path) ? new FileInfo(path).Length : 0;
                results.Add((folder, name, size > 1000, size, sw.ElapsedMilliseconds, null));
            }
        ...
        void Add(AyTeknikKatalog.Models.Catalog cat, int n)
        {
            for (int i = 0; i < n; i++)
            {
                ...
                var p = new AyTeknikKatalog.Models.Product
                {
                ...
                {
                    p.HasTable = true;
                    p.Table = new AyTeknikKatalog.Models.ProductTable
                    {
                        Title = d.TableTitle,
                        Specs =
                        {
                            new AyTeknikKatalog.Models.ProductSpec { Label = d.Spec1L, Value = d.Spec1V },
                            new AyTeknikKatalog.Models.ProductSpec { Label = d.Spec2L, Value = d.Spec2V },
                            new AyTeknikKatalog.Models.ProductSpec { Label = d.Spec3L, Value = d.Spec3V },
                            new AyTeknikKatalog.Models.ProductSpec { Label = d.Spec4L, Value = d.Spec4V },
                        }
                    };
                }
                cat.Products.Add(p);
        ...
        Run("01-cover", "02-fullpage-landscape", c => { if (cover != null) { c.Cover.CustomCoverImagePath = cover; c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.FullPage; } Add(c, 3); });
        Run("01-cover", "03-topbar-mode", c => { if (cover != null) { c.Cover.CustomCoverImagePath = cover; c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.TopWithBrandBar; } Add(c, 3); });
        Run("01-cover", "04-overlay-mode", c => { if (cover != null) { c.Cover.CustomCoverImagePath = cover; c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.BackgroundWithOverlay; } Add(c, 3); });
        ...
        // === 03-themes (23) ===
        foreach (var pt in AyTeknikKatalog.Models.PdfTheme.All) { var tt = pt.Id; Run("03-themes", $"theme-{tt}", c => { c.ThemeId = tt; Add(c, 6); }); }

        // === 04-edges (11) ===
        Run("04-edges", "01-empty-everything", c => { }, () => new AyTeknikKatalog.Models.Catalog());
        Run("04-edges", "02-turkish-chars", c => { c.Brand.Name = "ŞİĞÖÇÜ Şirketi"; c.Brand.About = "Çağdaş üretim, ölçülü işçilik, ığdırlı kalite"; c.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Türkçe ığİ Ürün", Code = "T-Ş-1", Price = 99.99m, Currency = "TL", ImagePath = photos[0] }); });
        ...
        Run("04-edges", "06-negative-price", c => { c.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Neg", Code = "N1", Price = -100m, Currency = "TL", ImagePath = photos[0] }); });
        Run("04-edges", "07-zero-price", c => { c.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Zero", Code = "Z1", Price = 0m, Currency = "TL", ImagePath = photos[0] }); });
        Run("04-edges", "08-very-low-price", c => { c.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Mikro", Code = "M1", Price = 0.001m, Currency = "TL", ImagePath = photos[0] }); });
        Run("04-edges", "09-EUR-currency", c => { c.Products.Add(new AyTeknikKatalog.Models.Product { Name = "Euro", Code = "E1", Price = 50m, Currency = "EUR", ImagePath = photos[0] }); });
        Run("04-edges", "10-empty-name", c => { c.Products.Add(new AyTeknikKatalog.Models.Product { Name = "", Code = "EMPTY", Price = 100m, Currency = "TL", ImagePath = photos[0] }); });
        Run("04-edges", "11-very-long-product-name", c => { c.Products.Add(new AyTeknikKatalog.Models.Product { Name = new string('X', 200), Code = "L1", Price = 100m, Currency = "TL", ImagePath = photos[0] }); });
        ...
        Run("05-real-world", "02-sample-12-custom-pages", c =>
        {
            ...
            c.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "1x1", ProductIds = { c.Products[0].Id } });
            c.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "1x2", ProductIds = { c.Products[1].Id, c.Products[2].Id } });
            c.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "2x2", ProductIds = { c.Products[3].Id, c.Products[4].Id, c.Products[5].Id, c.Products[6].Id } });
            c.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "3x3", ProductIds = { c.Products[7].Id, c.Products[8].Id, c.Products[9].Id, c.Products[10].Id, c.Products[11].Id } });
        });
        Run("05-real-world", "03-references-10", c => { for (int i = 1; i <= 10; i++) c.References.Add(new AyTeknikKatalog.Models.Reference { Name = $"Müşteri Firma {i}" }); Add(c, 6); });
        Run("05-real-world", "05-no-references-page", c => { c.SkipReferencesPage = true; for (int i = 1; i <= 10; i++) c.References.Add(new AyTeknikKatalog.Models.Reference { Name = $"M{i}" }); Add(c, 6); });

        // === 06-stress (4) ===
        Run("06-stress", "01-products-100", c => { c.LayoutId = "3x3"; Add(c, 100); });
        Run("06-stress", "02-products-500", c => { c.LayoutId = "4x4"; Add(c, 500); });
        Run("06-stress", "03-references-200", c => { for (int i = 1; i <= 200; i++) c.References.Add(new AyTeknikKatalog.Models.Reference { Name = $"Ref {i}" }); Add(c, 6); });
        Run("06-stress", "04-mixed-50-products-50-refs", c => { for (int i = 1; i <= 50; i++) c.References.Add(new AyTeknikKatalog.Models.Reference { Name = $"Ref {i}" }); Add(c, 50); });

        // === 07-singleproduct (3) ===
        RunSingle("07-singleproduct", "01-with-image", () => { var cat = AyTeknikKatalog.Models.Catalog.CreateDefault(); var p = new AyTeknikKatalog.Models.Product { Name = "Tek Ürün", Code = "S1", Price = 999m, Currency = "TL", ImagePath = photos[0] }; cat.Products.Add(p); return (cat, p); });
        RunSingle("07-singleproduct", "02-no-image", () => { var cat = AyTeknikKatalog.Models.Catalog.CreateDefault(); var p = new AyTeknikKatalog.Models.Product { Name = "Görselsiz", Code = "S2", Price = 199m, Currency = "TL" }; cat.Products.Add(p); return (cat, p); });
        RunSingle("07-singleproduct", "03-with-table", () =>
        {
            ...
            var p = new AyTeknikKatalog.Models.Product
            {
                ...
                HasTable = true,
                Table = new AyTeknikKatalog.Models.ProductTable { Title = "Teknik", Specs = { new AyTeknikKatalog.Models.ProductSpec { Label = "Çap", Value = "32mm" }, new AyTeknikKatalog.Models.ProductSpec { Label = "Basınç", Value = "16 bar" } } }
            };
            cat.Products.Add(p);
            return (cat, p);
        });
        ...
        // === 08-designs (8) ===
        foreach (var d in AyTeknikKatalog.Models.PdfDesign.All) { var did = d.Id; Run("08-designs", $"design-{did}", c => { c.DesignId = did; Add(c, 3); }); }
    ...
    private static void RunShowcase()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < productData.Length && i < photos.Count; i++)
            {
                ...
                cat.Products.Add(new AyTeknikKatalog.Models.Product
                {
            ...
            cat.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "1x1", ProductIds = { prods[2].Id } });
            cat.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "1x2", ProductIds = { prods[0].Id, prods[1].Id } });
            cat.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "2x2", ProductIds = { prods[3].Id, prods[4].Id, prods[5].Id, prods[6].Id } });
            cat.CustomPages.Add(new AyTeknikKatalog.Models.CustomPageEntry { LayoutId = "3x3", ProductIds = { prods[7].Id, prods[8].Id, prods[9].Id, prods[10].Id, prods[11].Id } });

            var output = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "ornek-showcase.pdf");
            new AyTeknikKatalog.Services.PdfService().Generate(cat, output);
    ...
    private static void RunComprehensiveTests()
    {
        ...
        void Test(string name, Action<AyTeknikKatalog.Models.Catalog> setup)
        {
            try
            {
                var cat = AyTeknikKatalog.Models.Catalog.CreateDefault();
        ...
        Test("P0-09 product with table", c => {
            ...
            prod.HasTable = true;
            prod.Table = new AyTeknikKatalog.Models.ProductTable
            {
                Title = "Teknik Özellikler",
                Specs = { new AyTeknikKatalog.Models.ProductSpec { Label = "Güç", Value = "2000W" } }
            };
            c.Products.Add(prod);
        });
        ...
        Test("P2b CoverMode=FullPage", c => { c.Cover.CustomCoverImagePath = c.Brand.LogoPath; c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.FullPage; });
        Test("P2b CoverMode=TopWithBrandBar", c => { c.Cover.CustomCoverImagePath = c.Brand.LogoPath; c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.TopWithBrandBar; });
        Test("P2b CoverMode=BackgroundWithOverlay", c => { c.Cover.CustomCoverImagePath = c.Brand.LogoPath; c.Cover.CustomCoverImageMode = AyTeknikKatalog.Models.CoverImageMode.BackgroundWithOverlay; });

        File.AppendAllText(resultFile, "\n[P3] Edge case'ler\n");
        Test("P3 Empty brand all blank", c => {
            c.Brand = new AyTeknikKatalog.Models.BrandInfo();
        });
        Test("P3 Brand only LogoPath set", c => {
            c.Brand = new AyTeknikKatalog.Models.BrandInfo { LogoPath = c.Brand.LogoPath };
        });
        ...
        Test("P3 50 references", c => {
            for (int i = 1; i <= 50; i++) c.References.Add(new AyTeknikKatalog.Models.Reference { Name = $"Ref {i}" });
        });

        File.AppendAllText(resultFile, "\n[P4] Sample + alternate paths\n");
        try
        {
            var single = AyTeknikKatalog.Models.Catalog.CreateDefault();
            var prod = MakeProduct("Single Product PDF", "S1", 250);
            single.Products.Add(prod);
            new AyTeknikKatalog.Services.PdfService().GenerateSingleProduct(single, prod, Path.Combine(outDir, "P4_SingleProduct.pdf"));
            passed++;
    ...
    private static AyTeknikKatalog.Models.Product MakeProduct(string name, string code, decimal price)
    {
        return new AyTeknikKatalog.Models.Product { Name = name, Code = code, Price = price, Currency = "TL" };
    }
    ...
    private static void RunVanaStressTest()
    {
        ...
        void AddVanaProducts(AyTeknikKatalog.Models.Catalog cat, int count)
        {
            var names = new[] {
        ...
        var modes = new[]
        {
            ("FullPage", AyTeknikKatalog.Models.CoverImageMode.FullPage),
            ("UstYari", AyTeknikKatalog.Models.CoverImageMode.TopWithBrandBar),
            ("SagYari", AyTeknikKatalog.Models.CoverImageMode.BackgroundWithOverlay),
        };
