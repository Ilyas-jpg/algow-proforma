...
> Sürüm: 1.6.0 · Windows 10/11 (x64) · Bu kılavuz son kullanıcı içindir; geliştirici kurulumu için [README.md](README.md), Gmail/Workspace bağlantısı için [GOOGLE_WORKSPACE_SETUP.md](GOOGLE_WORKSPACE_SETUP.md).
...
- **Excel'den İçe Aktar / Excel'e Aktar:** kütüphane penceresinin sağ üstünde — Excel listenizi katalogdan geçirmeden doğrudan kütüphaneye alır; kütüphaneyi kod/ad/fiyat/para birimi/kategori sütunlarıyla dışa verir.
...
2. Sayfa ve sütunları eşleyin (kod / ad / fiyat / **kategori** sütunu). Program sütunları otomatik tahmin eder; gerekirse düzeltin — eşlemeyi değiştirdiğiniz anda önizleme kendiliğinden yenilenir.
3. Önizlemeden onaylayın — ürünler (kategorileriyle) kataloğa eklenir. Kategori sütunu eşlerseniz [kategori başlık sayfaları](#kategori-başlık-sayfaları) doğrudan Excel'den beslenir.
...
- **Toplu zam:** *Tümüne Uygula* listenin tamamına, *Seçiliye Uygula* yalnız seçtiğiniz satırlara yüzde uygular (Ctrl/Shift ile çoklu satır seçin; +10 zam, −5 indirim).
- Teklif yazarken kalemleri bu havuzdan seçebilirsiniz — fiyatlar elle yazılmaz, havuzdan gelir. Havuz kaleminin para birimi tekliften farklıysa program sorar: TCMB günlük kuruyla çevirebilir, sayıyı aynen alabilir ya da vazgeçebilirsiniz.
...
**Teklif** düğmesi teklif editörünü açar (başlığın yanındaki rozet teklifin güncel **durumunu** gösterir):

- **Müşteri:** listeden seçip **Uygula** deyin; **+ Yeni** ile CRM'de olmayan müşteriyi editörden ayrılmadan ekleyebilirsiniz (kaydedilir ve teklife uygulanır). Firma/yetkili/e-posta/telefon alanları teklif üzerinde **düzenlenebilir** — değişiklik yalnız bu teklifi etkiler, müşteri kartına dokunmaz.
- **Kalemler:** ad, açıklama, miktar, birim, birim fiyat, satır iskontosu (%), KDV oranı. Fiyat havuzundan seçerek de ekleyebilirsiniz; kalemin para birimi tekliften farklıysa program uyarır ve isterseniz TCMB kuruyla çevirir.
...
- **Dil:** `Türkçe` veya `English` — EN seçilirse tüm PDF etiketleri İngilizce basılır (başlık **PROFORMA INVOICE**), sayı biçimi `1,234.56`, tarih `15 Jun 2026` biçiminde, TL tutarlar uluslararası `TRY` koduyla gösterilir; ihracat müşterisi için birebir.
...
Ana penceredeki **Teklifler** düğmesi kayıtlı tüm teklifleri listeler: numara, tarih, tutar ve **durum** (Taslak / Gönderildi / Onaylandı / Reddedildi). Başlıkta özet sayaç ve **onaylanan tekliflerin toplam tutarı** (para birimi kırılımıyla — TL/USD/EUR ayrı) görünür. Üstteki **arama kutusu** numara/firma/yetkili/e-postada arar; **Durum** süzgeciyle yalnız belirli durumdakileri listelersiniz.

- Durumu listeden değiştirdiğiniz anda kaydedilir; e-postayla başarıyla gönderilen teklif **kendiliğinden** Taslak → Gönderildi olur.
- **Aç** teklifi editörde açar, **PDF** üretilmiş PDF'i gösterir.
- **Sil** teklifi (varsa PDF'iyle) kaldırır — kalıcı silinmez, 30 gün `Teklifler\.trash` klasöründe geri alınabilir durur.
- **Gönderim Geçmişi** düğmesi tüm e-posta gönderimlerinin kaydını açar: tarih, teklif no, alıcı, kanal (Gmail/SMTP), sonuç ve hata mesajı; arama + *yalnız başarısızlar* süzgeci vardır.
...
3. **Önizleme zorunludur** — *Önizle & Gönder*'e bastığınızda program ilk alıcının adına **gerçek örnek PDF üretir** ve kişiselleştirilmiş konu/gövde/ek adıyla birlikte gösterir; **Örnek PDF'i Aç** ile ekin birebir aynısını inceleyebilirsiniz. Gönderim ancak bu pencerede **Gönderimi Başlat** derseniz başlar.
4. Program her alıcı için teklifi kopyalar, müşteri bilgisini işler, PDF üretir ve sırayla gönderir; **İptal** düğmesi süren gönderimi sıradaki alıcıda durdurur.

Güvenlik ve hijyen:

- **Hız sınırı:** dakikada gönderim adedi Ayarlar'dan (varsayılan 20) — spam filtrelerine takılmamak için.
- **Gönderim logu** tutulur (kime, ne zaman, hangi teklif, sonuç) — Teklifler panosundaki **Gönderim Geçmişi** penceresinden izlenir; log yıl bazında arşivlenir.
...
| **Veri Yedeği** | **Yedek Al (ZIP)** tüm verinizi tek dosyaya alır; **Yedekten Geri Yükle** geri getirir (mevcut veri silinmez, güvenlik kopyasına taşınır) ve uygulama taze veriyle **otomatik yeniden başlar** |
...
**Yedekleme (önerilen yol):** Ayarlar → **Veri Yedeği → Yedek Al (ZIP)** tüm veriyi tek dosyaya alır; **Yedekten Geri Yükle** ile geri getirirsiniz (mevcut veri silinmez, `pre-restore` güvenlik kopyasına taşınır; geri yükleme bitince uygulama taze veriyle kendiliğinden yeniden başlar). Elle yedek isterseniz `Belgeler\Algow Proforma Kataloglar` klasörünü kopyalamak da yeterlidir. `.bin` dosyaları başka makinede çözülemez (tasarım gereği) — yeni makinede şifre/Google bağlantısını bir kez yeniden girersiniz.
