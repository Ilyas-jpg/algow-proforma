...
- Kaydedilen teklifler `Belgeler\Algow Proforma Kataloglar\Teklifler` altında durur; editörden tekrar açıp **revizyon** olarak güncelleyebilirsiniz.
...
2. Alıcıları müşteri listenizden işaretleyin (hitap ve ad şablona otomatik girer).
...
**Yedekleme:** `Belgeler\Algow Proforma Kataloglar` klasörünü kopyalamanız yeterlidir. `.bin` dosyaları başka makinede çözülemez (tasarım gereği) — yeni makinede şifre/Google bağlantısını bir kez yeniden girersiniz.
